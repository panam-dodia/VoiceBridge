...
export default function YouTubePage() {
  ...
  const detectGender = async () => {
    ...
    try {
      // Use first few segments for gender detection
  ...
  const playAudio = async (translatedText: string, index: number) => {
    try {
  ...
  // Detect gender when transcript loads
  useEffect(() => {
          ...
          <div className="grid lg:grid-cols-2 gap-6 mb-6">
            ...
            <div className="bg-white/5 backdrop-blur-sm rounded-2xl border border-white/10 p-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-xl font-bold text-white">Translation ({targetLanguage})</h2>
                  ...
                  >
                    {translations[index] ? (
                      <div className="flex items-start gap-3">
                        <p className="text-white flex-1">{translations[index]}</p>
                        <button
                          onClick={() => {
                            if (isPlayingAudio && currentAudioIndex === index) {
                              stopAudio();
                            } else {
                              playAudio(translations[index], index);
                            }
                          }}
                        ...
                        >
                          {isPlayingAudio && currentAudioIndex === index ? '⏸ Stop' : '▶ Play'}
                        </button>
                      </div>
                    ) : (
